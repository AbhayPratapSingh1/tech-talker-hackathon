...
const ChatPage = ({ page, user, socketRef, selectedId }) => {
    ...
    function replaceText() {
        ...
        let textArray = textInput.toLowerCase();

        for (let i =0 ; i < Object.keys(commonSentenceObj).length; i++){
            let word = Object.keys(commonSentenceObj)[i]?.toLowerCase()
            if (textArray.includes(word)){
                console.log("yeess")
                textArray.replace(word,commonSentenceObj[word])
        ...
        for (let i = 0; i < textArray.length; i++) {
            ...
            if (key) {
                ...
                key = Object.keys(emojiObj).find(each => (each.toLowerCase().startsWith(word) || word.startsWith(each.toLowerCase())) && Math.abs(each.length - word.length ) < 2);
                if (key) {
                    text += emojiObj[key];
                }
            }
